...
!#############################################################
      ...
      cdum=trim(cHOME)//'/'//trim(cINP)//'/coefficient_JEJU.inp'
      open(unit=12,file=trim(cdum),status='old')
      !open(unit=12,file='coefficient_JEJU.inp',status='old')
      read(12,*)
      do i=0,im
      ...
      cdum=trim(cHOME)//'/'//trim(cINP)//'/out_jeju_temp_utm_all.csv'
...
!#############################################################
      ...
      if (t.lt.0. .or. t.gt.40.) then
      	...
      	write(cqout,'(6a)') trim(cHOME),'/',trim(cLOG),"/QCJEJU_",ctoday,".LOG"
