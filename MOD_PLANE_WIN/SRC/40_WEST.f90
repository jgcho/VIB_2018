...
!#############################################################
      ...
      cdum=trim(cHOME)//'/'//trim(cINP)//'/coefficient_WEST.inp'
      open(unit=12,file=trim(cdum),status='old')
      !open(unit=12,file='coefficient_WEST.inp',status='old')
      read(12,*)
      do i=0,im
      ...
      cdum=trim(cHOME)//'/'//trim(cINP)//'/out_west_temp_utm_all.csv'
...
!#############################################################
      ...
      if (t.lt.0. .or. t.gt.40.) then
      	...
      	write(cqout,'(6a)') trim(cHOME),'/',trim(cLOG),"/QCWEST_",ctoday,".LOG"
